...
using Dtg.Blazor.Server.Services;
    ...
    public class Startup {
        ...
        public void ConfigureServices(IServiceCollection services) {
            ...
            services.AddXafSecurity(options => {
                ...
                options.UserType = typeof(Dtg.Module.BusinessObjects.ApplicationUser);
                // ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
                // If you use PermissionPolicyUser or a custom user type, comment out the following line:
                options.UserLoginInfoType = typeof(Dtg.Module.BusinessObjects.ApplicationUserLoginInfo);
                ...
                options.Events.OnAuthenticated = (externalAuthenticationContext) => {
                    ...
                    externalAuthenticationContext.Principal.Identity.AuthenticationType != SecurityDefaults.WindowsAuthentication && !(externalAuthenticationContext.Principal is WindowsPrincipal)) {
                        ...
                        object CreateApplicationUser(string userName, string providerUserId) {
                            if(objectSpace.FirstOrDefault<Dtg.Module.BusinessObjects.ApplicationUser>(user => user.UserName == userName) != null) {
                                throw new ArgumentException($"The username ('{userName}') was already registered within the system");
                            }
                            var user = objectSpace.CreateObject<Dtg.Module.BusinessObjects.ApplicationUser>();
                        ...
                        ISecurityUserLoginInfo FindUserLoginInfo(string loginProviderName, string providerUserId) {
                            return objectSpace.FirstOrDefault<Dtg.Module.BusinessObjects.ApplicationUserLoginInfo>(userLoginInfo =>
            ...
            services.AddDbContextFactory<Dtg.Module.BusinessObjects.DtgEFCoreDbContext>((serviceProvider, options) => {
            ...
            services.AddSwaggerGen(c => {
                ...
                c.SwaggerDoc("v1", new OpenApiInfo {
                    Title = "Dtg API",
                    Version = "v1",
                    Description = @"Use AddXafWebApi(options) in the Dtg.Blazor.Server\Startup.cs file to make Business Objects available in the Web API."
                });
                    ...
                    Flows = new OpenApiOAuthFlows() {
                        AuthorizationCode = new OpenApiOAuthFlow() {
                            ...
                            Scopes = new Dictionary<string, string> {
                                ...
                                { @"[Enter the scope name in the Dtg.Blazor.Server\Startup.cs file]", @"[Enter the scope description in the Dtg.Blazor.Server\Startup.cs file]" }
                            }
                        }
                    }
                });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
            if(env.IsDevelopment()) {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => {
                    c.SwaggerEndpoint("/swagger/v1/swagger.json", "Dtg WebApi v1");
